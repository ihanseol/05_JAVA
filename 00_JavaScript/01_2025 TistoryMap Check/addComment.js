function addComment(submitButton, entryId) {
    (function ($) {
        ...
        if (typeof data.comment === 'string' && data.comment.length > MAX_COMMENT_SIZE) {
            alert('�볤��� ' + MAX_COMMENT_SIZE + '�먭퉴吏� �낅젰�� �� �덉뒿�덈떎.');
            commentInput && commentInput.focus();
            return;
        }

        if (data.secret === 1 && T.config.ROLE === 'guest') {
            if (confirm('鍮꾨줈洹몄씤 �볤��� 怨듦컻 �묒꽦留� 媛��ν빀�덈떎. 濡쒓렇�� �섏떆寃좎뒿�덇퉴?')) {
...
function followBlog(blogId, $target, url, device) {
    if (!!initData.user) {
        ...
        return tjQuery.ajax({
            ...
            tjQuery(".btn_subscription").attr('data-tiara-copy', '援щ룆以�');
...
function unfollowBlog(blogId, $target, url, device) {
    if (!!initData.user) {
        ...
        tjQuery.ajax({
            ...
            tjQuery(".btn_subscription").attr('data-tiara-copy', '援щ룆�섍린');
